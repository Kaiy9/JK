...
const initWaline = async () => {
  ...
  try {
    ...
    walineInstance = init({
      el: walineRef.value,
      serverURL: 'https://waline.jieliu.xyz/',
  ...
  if (typeof window !== 'undefined') {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (nickInput) nickInput.setAttribute('placeholder', '台下何人')
        if (mailInput) mailInput.setAttribute('placeholder', '所诉何求')
        if (linkInput) linkInput.setAttribute('placeholder', '如何找你')
      }, 500)
    }, 200)
  }
})
