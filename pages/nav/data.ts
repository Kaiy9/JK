// @ts-nocheck
import type { NavData } from '../.vitepress/theme/types'

export const NAV_DATA: NavData[] = [
  {
    title: '娱乐休闲',
    items: [
      {
        icon: 'https://tophub.today/favicon.ico',
        title: '今日热榜',
        desc: '聚合每日各个网站的热榜',
        link: 'https://tophub.today/',
      },
      {
        title: 'V2EX',
        icon: 'https://www.v2ex.com/static/icon-192.png',
        desc: '一个关于分享和探索的地方',
        link: 'https://www.v2ex.com',
      },
      {
       title: "Google 地球",
       desc: "Google 开发的虚拟地球仪探索工具",
       link: "https://earth.google.com",
       icon: "https://img.51pgzs.com/22/imgfile/20230826/26145200ndf8.png"
     },
      {
      title: "微信文件传输",
      icon: "https://logo.clearbit.com/filehelper.weixin.qq.com",
      desc: "微信文件传输",
      link: "https://filehelper.weixin.qq.com/",
    },
    {
        icon: 'https://momoyu.cc/icon-192.png',
        title: '摸鱼热榜',
        desc: '摸鱼热榜',
        link: 'https://momoyu.cc',
      },
      {
        icon: "https://getzlib.com/favicon.ico",
        title: '获取 Z-Library',
        desc: '查找最新的 Z-Library 可用链接',
        link: 'https://getzlib.com/zh',
      },
      {
        icon: 'http://www.88kanqiu.love/storage/images/d0170ac257b781a7d9510c4c918e9e0c.svg',
        title: '88看球',
        desc: '免费看球的网站',
        link: 'http://www.88kanqiu.love/',
      },
      {
        icon: 'https://v.qq.com/favicon.ico',
        title: '腾讯视频',
        desc: '企鹅视频',
        link: 'https://v.qq.com',
      },
      {
        icon: 'https://static.hdslb.com/mobile/img/512.png',
        title: '哔哩哔哩',
        desc: '变质的网站',
        link: 'https://www.bilibili.com',
      },
      {
        icon: 'https://global-uploads.webflow.com/59deb588800ae30001ec19c9/5d4891e0e260e3c1bc37b100_beautiful%20ai%20favicon%20%20blue%20square.png',
        title: 'Beautiful.ai（PPT）',
        desc: 'AI驱动的演示文稿制作工具，智能生成专业PPT',
        link: 'https://www.beautiful.ai',
      },
      {
        title: 'AI工具集',
        icon: 'https://ai-bot.cn/wp-content/uploads/2023/07/ai-bot-favicon.png',
        desc: '国内外AI工具集合网站大全',
        link: 'https://ai-bot.cn/',
      },
      {
        icon: '/icons/pixiv.png',
        title: 'Pixiv',
        desc: '你懂得',
        link: 'https://www.pixiv.net',
      },
      {
        icon: 'https://www.youtube.com/s/desktop/014dbbed/img/favicon_48x48.png',
        title: 'YouTube',
        desc: '油管',
        link: 'https://www.youtube.com',
      },
      {
        icon: 'https://netflix.com/favicon.ico',
        title: 'Netflix',
        desc: '耐飞',
        link: 'https://netflix.com',
      },
    ],
  },
  {
    title: 'AI 工具',
    items: [
      {
        icon: 'https://chat.openai.com/favicon.ico',
        title: 'ChatGPT',
        desc: '万恶之源',
        link: 'https://chat.openai.com',
      },
      {
        icon: 'https://manus.im/favicon.ico',
        title: 'manus',
        desc: 'manus 的自动机器人',
        link: 'https://manus.im/login?code=Q8ZFSCLPVCFULHA&type=signUp',
      },
      {
        icon: 'https://gemini.google.com/logo.svg',
        title: 'Gemini',
        desc: '白嫖只能支持了 干翻gpt',
        link: 'https://gemini.google.com',
      },
      {
        icon: 'https://grok.x.ai/favicon.ico',
        title: 'Grok',
        desc: 'xAI 的生成式人工智能聊天机器人',
        link: 'https://grok.x.ai',
      },
      {
        badge: '笔记',
        icon: 'https://www.notion.so/images/logo-ios.png',
        title: 'Notion AI',
        desc: 'AI增强的笔记和知识管理平台',
        link: 'https://www.notion.so',
      },
      {
        desc: '绘画',
        icon: 'https://www.midjourney.com/apple-touch-icon.png',
        title: '还凑合的绘画Midjourney',
        link: 'https://www.midjourney.com',
      },
      {
        desc: '绘画',
        icon: 'https://global-uploads.webflow.com/59deb588800ae30001ec19c9/5d4891e0e260e3c1bc37b100_beautiful%20ai%20favicon%20%20blue%20square.png',
        title: 'Beautiful.ai（PPT）',
        link: 'https://www.beautiful.ai',
      },
      {
        badge: '绘画',
        icon: 'https://stablediffusionweb.com/favicon.ico',
        title: 'Stable Diffusion',
        desc: '开源的AI图像生成模型，支持文本到图像转换',
        link: 'https://stablediffusionweb.com',
      },
      {
        icon: 'https://img.js.design/assets/webImg/favicon.ico',
        title: '即时设计',
        desc: '同时创造，即时设计',
        link: 'https://js.design/',
      },
      {
        icon: 'https://www.midjourney.com/apple-touch-icon.png',
        title: 'Midjourney（绘画）',
        link: 'https://www.midjourney.com',
        desc: '顶级的 AI 图像生成工具',
      },
      {
        icon: 'https://assets.website-files.com/60de2701a7b28f308f619d3d/62f5b1528499d8e6b3d02447_Gamma_V1_Icon_only_4.gif',
        title: 'Gamma (PPT)',
        link: 'https://gamma.app/',
        desc: 'AI 驱动，快速生成演示文稿和网页',
      },
      {
      title: "Perplexity",
      badge: "在线",
      icon: "https://www.perplexity.ai/favicon.ico",
      desc: "AI驱动的搜索和问答平台，提供实时信息",
      link: "https://www.perplexity.ai/",
    },
    {
      title: "NotebookLM",
      badge: "在线",
      icon: "https://notebooklm.google.com/_/static/branding/v4/light_mode/favicon/favicon.svg",
      desc: "Google的AI笔记助手，基于文档进行智能对话",
      link: "https://notebooklm.google.com/",
    },
      {
        icon: 'https://claude.ai/favicon.ico',
        title: 'Claude',
        link: 'https://claude.ai',
        desc: 'Anthropic 公司的大型语言模型，代码辅助能力强',
      },
    ],
  },
  {
    title: '社交媒体',
    items: [
      {
        icon: 'https://instagram.com/favicon.ico',
        title: 'Instagram',
        desc: '在线图片及视频分享的社群应用',
        link: 'https://instagram.com',
      },
      {
        icon: 'https://www.xiaohongshu.com/favicon.ico',
        title: '小红书',
        desc: '查资料挺方便的',
        link: 'https://www.xiaohongshu.com/',
      },
      {
        icon: 'https://weibo.com//favicon.ico',
        title: '微博',
        desc: '微博',
        link: 'https://weibo.com/',
      },
      {
        icon: 'https://x.com/favicon.ico',
        title: 'X',
        desc: '全球顶尖的社交媒体平台之一',
        link: 'https://x.com',
      },
      {
        icon: 'https://reddit.com/favicon.ico',
        title: 'Reddit',
        desc: '娱乐、社交及新闻网站',
        link: 'https://reddit.com',
      },
    ],
  },
  {
    title: '电子邮件',
    items: [
      {
        icon: 'https://outlook.com/favicon.ico',
        title: 'Outlook',
        desc: '微软的免费互联网收发电子邮件服务',
        link: 'https://outlook.com',
      },
      {
        icon: 'https://gmail.com/favicon.ico',
        title: 'Gmail',
        desc: 'Google 的免费电子邮件服务',
        link: 'https://gmail.com',
      },
      {
      title: "QQ邮箱",
      icon: "https://logo.clearbit.com/mail.qq.com",
      desc: "腾讯提供的免费邮箱服务",
      link: "https://mail.qq.com/",
    },
     {
      title: "互联网档案馆",
      icon: "https://logo.clearbit.com/archive.org",
      desc: "保存互联网历史记录和数字文化遗产",
      link: "https://archive.org/",
   },
    {
      title: "PayPal美国",
      icon: "https://logo.clearbit.com/www.paypal.com",
      desc: "全球领先的在线支付平台",
      link: "https://www.paypal.com/us/home",
    },
    {
      title: "Apple ID美国",
      icon: "https://logo.clearbit.com/appleid.apple.com",
      desc: "苹果账户管理平台",
      link: "https://appleid.apple.com/us/",
    },
    {
      title: "邮箱测试",
      icon: "https://logo.clearbit.com/www.mail-tester.com",
      desc: "检测邮箱配置和发送能力的专业工具",
      link: "https://www.mail-tester.com/",
    },
    {
      title: "Snapmail",
      icon: "https://logo.clearbit.com/www.snapmail.cc",
      desc: "临时邮箱服务，保护隐私",
      link: "https://www.snapmail.cc/",
    },
    {
      title: "临时邮箱",
      icon: "https://logo.clearbit.com/www.linshiyouxiang.net",
      desc: "提供临时邮箱地址，避免垃圾邮件",
      link: "https://www.linshiyouxiang.net/",
    },
      {
        icon: 'https://temp-mail.org/favicon.ico',
        title: 'Temp Mail',
        desc: '匿名的一次性电子邮件',
        link: 'https://temp-mail.org',
      },
    ],
  },
  {
    title: '网络监控',
    items: [
      {
        icon: 'https://fast.com/favicon.ico',
        title: 'Fast',
        desc: '奈飞的简单网速测试',
        link: 'https://fast.com',
      },
      {
        icon: 'https://whoer.net/favicon.ico',
        title: 'Whoer',
        desc: 'IP 伪装度检测',
        link: 'https://whoer.net',
      },
       {
      title: "IP.SB",
      icon: "https://logo.clearbit.com/ip.sb",
      desc: "IP地址查询和网络信息检测工具",
      link: "https://ip.sb/",
    },
    {
      title: "IP111",
      icon: "https://logo.clearbit.com/ip111.cn",
      desc: "IP地址查询和地理位置定位服务",
      link: "http://ip111.cn/",
    },
      {
        icon: 'https://ping0.cc/favicon.ico',
        title: 'PING0',
        desc: 'IP 纯净度检测',
        link: 'https://ping0.cc',
      },
      {
        icon: 'https://ipcheck.ing/favicon.ico',
        title: 'IPCheck',
        desc: '好用和开源的全能 IP 工具箱',
        link: 'https://ipcheck.ing',
      },
    ],
  },
  {
    title: '一些常用的网站',
    items: [
      {
      title: "NO视频",
      icon: "https://logo.clearbit.com/www.novipnoad.net",
      desc: "免费在线视频观看平台",
      link: "https://www.novipnoad.net/",
    },
    {
      title: "低端影视",
      icon: "https://logo.clearbit.com/ddys.pro",
      desc: "高清影视资源在线观看",
      link: "https://ddys.pro/",
    },
      {
      title: "在线工具",
      icon: "https://logo.clearbit.com/tool.lu",
      desc: "开发者常用工具集合",
      link: "https://tool.lu/",
    },
      {
      title: "抠图",
      icon: "https://logo.clearbit.com/www.remove.bg",
      desc: "AI智能去除图片背景工具",
      link: "https://www.remove.bg/",
    },
      {
      title: "草料二维码生成器",
      icon: "https://logo.clearbit.com/cli.im",
      desc: "在线二维码生成和扫描工具",
      link: "https://cli.im/",
    },
    {
      title: "在线之家",
      icon: "https://logo.clearbit.com/www.zxzj.pro",
      desc: "在线影视资源观看平台",
      link: "https://www.zxzj.pro/",
    },
     {
      title: "美国地址生成",
      icon: "https://logo.clearbit.com/www.haoweichi.com",
      desc: "生成美国地址信息工具",
      link: "http://www.haoweichi.com/",
    },
    {
      title: "信用卡生成器",
      icon: "https://logo.clearbit.com/ccardgenerator.com",
      desc: "生成测试用信用卡号码",
      link: "https://ccardgenerator.com/",
    },
    {
      title: "伪名称生成器",
      icon: "https://logo.clearbit.com/www.fakenamegenerator.com",
      desc: '身份',
      link: "https://www.fakenamegenerator.com/",
    },
    {
      title: "美国身份证生成",
      icon: "https://logo.clearbit.com/www.ssnzk.com",
      desc: "生成美国身份证信息",
      link: "https://www.ssnzk.com/",
    },
       {
      title: "云短信",
      icon: "https://logo.clearbit.com/yunduanxin.net",
      desc: "在线短信接收平台",
      link: "https://yunduanxin.net/",
    },
    {
      title: "SMS-Activate",
      icon: "https://logo.clearbit.com/sms-activate.io",
      desc: "国际短信接收服务",
      link: "https://sms-activate.io/",
    },
    {
      title: 'Google 地球',
      icon: 'https://img.51pgzs.com/22/imgfile/20230826/26145200ndf8.png',
      desc: 'Google 开发的虚拟地球仪探索工具，叠加高清卫星影像与丰富地理数据，支持多维度交互方式探索地球全貌的专业平台',
      link: 'https://earth.google.com',
    },
    {
      title: 'AI Box',
      icon: 'https://latentbox.com/favicon.ico',
      desc: '汇集全球优秀 AI 应用工具的产品目录平台，涵盖创作设计办公等多个专业领域，助力用户高效发现实用人工智能服务',
      link: 'https://latentbox.com',
    },
    {
      title: '智能着色',
      icon: 'https://zcoloring.com/favicon.ico',
      desc: '基于文字描述的 AI 绘画创作工具平台，能够快速生成独特个性化着色页面，满足用户创意绘画设计与个性化定制的多样需求',
      link: 'https://zcoloring.com',
    },
    {
      title: '稿定社区',
      icon: 'https://www.gaoding.art/favicon.ico',
      desc: '依托 AI 技术的图片创意设计制作平台，支持一键同款智能功能，轻松实现图文内容的高效创作与专业级视觉设计制作',
      link: 'https://www.gaoding.art',
    },
    {
      title: 'Vidu 视频',
      icon: 'https://www.vidu.cn/logo.svg',
      desc: 'AI 驱动的图片动态化视频处理工具，用户输入图文描述即可生成高质量创意视频内容，操作简便制作效率极高',
      link: 'https://www.vidu.cn',
    },
    {
      title: '魔方简历',
      icon: 'https://magicv.art/favicon.ico',
      desc: '智能简历生成制作服务平台，根据个人技能背景与求职方向需求，一键生成适配目标岗位的专业简历模板与内容',
      link: 'https://magicv.art/zh',
    },
    {
      title: '老鱼简历',
      icon: 'https://pic.52112.com/2019/08/08/EPS-190808_149/rINjxTXcTy_small.jpg',
      desc: '在线简历制作设计服务工具，用户输入个人信息与岗位类型需求，快速生成高质量定制化的专业求职简历文档',
      link: 'https://www.laoyujianli.com',
    },
    {
      title: '凹凸工坊',
      icon: 'https://www.autohanding.com/img/logo.a5f914db.png',
      desc: 'AI 智能转换电子文档为逼真手写体效果工具，支持多种个性化笔迹风格自定义设置，轻松生成各类手写作业文件和文档',
      link: 'https://www.autohanding.com',
    },
    {
      title: 'ChatExcel',
      icon: 'https://pic1.zhimg.com/100/v2-ed450048c00dd4515c1d19103925d00c_r.jpg',
      desc: '智能表格处理分析服务平台，用户上传数据后通过自然语言对话交互完成 Excel 操作与数据分析任务，操作简单高效',
      link: 'https://www.chatexcel.com',
    },
    {
      title: 'ProcessOn',
      icon: 'https://www.processon.com/favicon.ico',
      desc: 'AI 辅助绘图设计制作工具，根据用户具体需求一键生成各类专业流程图表，支持团队在线协作功能，显著提升可视化效率',
      link: 'https://www.processon.com/diagraming/680f3ab01780bd6627ec3ee1',
    },
    {
      title: 'GraphMaker',
      icon: 'https://aigraphmaker.net/sites/default/files/logo.svg',
      desc: 'AI 辅助绘图设计制作工具，根据用户具体需求一键生成各类专业流程图表，支持团队在线协作功能，显著提升可视化效率',
      link: 'https://aigraphmaker.net',
    },
    {
      title: 'Drawio',
      icon: 'https://img.foxmac.com/2021/03/draw.io.png',
      desc: '免费开源的跨平台图表绘制设计工具，支持团队在线协作绘制流程图 UML 图等，兼容多种格式文件与云存储服务',
      link: 'https://app.diagrams.net',
    },
    {
      title: 'Drawdb',
      icon: 'https://www.drawdb.app/favicon.ico',
      desc: '开源在线数据库设计建模专业工具，提供直观可视化建模与 SQL 代码自动生成功能，支持多类型数据库系统的定制开发',
      link: 'https://www.drawdb.app/editor',
    },
    {
      title: 'Wallspic',
      icon: 'https://www.1zj.com/uploadfile/2019/1011/20191011020039136.jpg',
      desc: '海量 4K8K 超清壁纸资源收藏库，每日更新全球顶级摄影师优质作品，支持按风格色彩主题分类进行精准搜索筛选',
      link: 'https://wallspic.com/cn/album',
    },
    {
      title: 'Wallhaven',
      icon: 'https://www.1zj.com/uploadfile/2019/1011/20191011020039726.jpg',
      desc: '海量优质 4K8K 分辨率壁纸资源平台，每日更新最受欢迎壁纸热门排行榜，涵盖动漫自然艺术等丰富主题分类',
      link: 'https://wallhaven.cc',
    },
    {
      title: 'Removebg',
      icon: 'https://www.remove.bg/favicon.ico',
      desc: 'AI 一键自动去除图片背景智能处理工具，精准识别主体边缘轮廓细节，支持人像商品等复杂场景的专业无痕抠图',
      link: 'https://www.remove.bg/zh',
    },
    {
      title: "厂长资源影视",
      icon: "https://logo.clearbit.com/czzy.top",
      desc: "高清影视资源下载平台",
      link: "www.czzymovie.com",
    },
    {
      title: "Porn Dude导航",
      icon: "https://logo.clearbit.com/theporndude.com",
      desc: "成人内容导航网站",
      link: "https://theporndude.com/zh",
    },
     {
      title: "指纹检测",
      icon: "https://logo.clearbit.com/www.yalala.com",
      desc: "浏览器指纹检测工具",
      link: "https://www.yalala.com/",
    },
    {
      title: "蓝奏云",
      icon: "https://logo.clearbit.com/pc.woozooo.com",
      desc: "免费云存储和文件分享平台",
      link: "https://pc.woozooo.com/",
    },
    {
      title: "夸克网盘",
      icon: "https://logo.clearbit.com/pan.quark.cn",
      desc: "阿里旗下的云存储服务",
      link: "https://pan.quark.cn/",
    },
    {
      title: "阿里云盘",
      icon: "https://logo.clearbit.com/www.aliyundrive.com",
      desc: "阿里巴巴提供的云存储服务",
      link: "https://www.aliyundrive.com/",
    },
    ],
  },
  {
    title: '一些社区',
    items: [
      {
        title: '稀土掘金',
        icon: 'https://lf3-cdn-tos.bytescm.com/obj/static/xitu_juejin_web//static/favicons/apple-touch-icon.png',
        desc: '面向全球中文开发者的技术内容分享与交流平台',
        link: 'https://juejin.cn',
      },
       {
        icon: 'https://telegram.org/favicon.ico',
        title: 'Telegram',
        desc: '跨平台的即时通信软件',
        link: 'https://telegram.org',
      },
    {
      title: "终身学习",
      icon: "https://logo.clearbit.com/lifelonglearn.ing",
      desc: "持续学习和知识分享平台",
      link: "https://lifelonglearn.ing/",
    },
    {
      title: "计算机教育中缺失的一课",
      icon: "https://logo.clearbit.com/missing-semester-cn.github.io",
      desc: "MIT计算机科学教育课程中文版",
      link: "https://missing-semester-cn.github.io/",
    },
    {
      title: "小孙同学",
      icon: "https://logo.clearbit.com/blog.sunguoqi.com",
      desc: "前端开发者的个人博客",
      link: "https://blog.sunguoqi.com/",
    },
    {
      title: "CrazyWong",
      icon: "https://logo.clearbit.com/crazywong.com",
      desc: "技术博客和开发经验分享",
      link: "https://crazywong.com/",
    },
    {
      title: "Keep",
      icon: "https://logo.clearbit.com/keep-docs.xpoet.cn",
      desc: "前端开发文档和教程",
      link: "https://keep-docs.xpoet.cn/",
    },
    {
      title: "PicGo",
      icon: "https://logo.clearbit.com/picgo.github.io",
      desc: "图床上传工具，支持多种云存储",
      link: "https://picgo.github.io/PicGo-Doc/",
    },
    {
      title: "LX Music",
      icon: "https://logo.clearbit.com/lxmusic.toside.cn",
      desc: "免费音乐播放器，支持多平台",
      link: "https://lxmusic.toside.cn/",
    },
    {
      title: "AList文档",
      icon: "https://logo.clearbit.com/alist.nn.ci",
      desc: "文件列表程序，支持多种存储",
      link: "https://alist.nn.ci/zh/",
    },
    {
      title: "IPTVindex",
      icon: "https://logo.clearbit.com/iptvindex.com",
      desc: "IPTV频道索引和直播源",
      link: "https://iptvindex.com/",
    },
    {
      title: "CS自学指南",
      icon: "https://logo.clearbit.com/csdiy.wiki",
      desc: "计算机科学自学资源集合",
      link: "https://csdiy.wiki/",
    },
    {
      title: "TrackersList",
      icon: "https://logo.clearbit.com/trackerslist.com",
      desc: "BitTorrent Tracker服务器列表",
      link: "https://trackerslist.com/",
    },
    {
      title: "大数据知识体系",
      icon: "https://logo.clearbit.com/www.sqlboy.tech",
      desc: "大数据技术学习和知识体系",
      link: "https://www.sqlboy.tech/",
    },
    {
      title: "程序员做饭指南",
      icon: "https://logo.clearbit.com/cook.aiursoft.cn",
      desc: "程序员的生活技能指南",
      link: "https://cook.aiursoft.cn/",
    },
    {
      title: "PagerMaid-Pyro",
      icon: "https://logo.clearbit.com/xtaolabs.com",
      desc: "Telegram用户机器人框架",
      link: "https://xtaolabs.com/",
    },
    {
      title: "LearnData 开源笔记",
      icon: "https://logo.clearbit.com/newzone.top",
      desc: "开源学习笔记和知识分享",
      link: "https://newzone.top/",
    },
    {
      title: "vuepress-theme-vdoing",
      icon: "https://logo.clearbit.com/doc.xugaoyi.com",
      desc: "VuePress主题和文档模板",
      link: "https://doc.xugaoyi.com/",
    },
    {
      title: "Reddit",
      icon: "https://logo.clearbit.com/www.reddit.com",
      desc: "全球最大的社区论坛",
      link: "https://www.reddit.com/",
    },
    {
      title: "MT论坛",
      icon: "https://logo.clearbit.com/bbs.binmt.cc",
      desc: "移动端开发技术论坛",
      link: "https://bbs.binmt.cc/",
    },
    {
      title: "吾爱破解",
      icon: "https://logo.clearbit.com/www.52pojie.cn",
      desc: "软件逆向分析技术论坛",
      link: "https://www.52pojie.cn/",
    },
    {
      title: "恩山论坛",
      icon: "https://logo.clearbit.com/www.right.com.cn",
      desc: "路由器固件和网络技术论坛",
      link: "https://www.right.com.cn/forum/",
    },
    {
      title: "科技玩家",
      icon: "https://logo.clearbit.com/www.kejiwanjia.com",
      desc: "科技资讯和数码产品论坛",
      link: "https://www.kejiwanjia.com/",
    },
    {
      title: "法海之路",
      icon: "https://logo.clearbit.com/fahai.org",
      desc: "法律知识学习和分享平台",
      link: "https://fahai.org/",
    },
    {
      title: "LINUX DO",
      icon: "https://logo.clearbit.com/linux.do",
      desc: "Linux技术学习和交流社区",
      link: "https://linux.do/",
    },
    {
      title: "NodeSeek",
      icon: "https://logo.clearbit.com/www.nodeseek.com",
      desc: "服务器和VPS技术交流论坛",
      link: "https://www.nodeseek.com/",
    },
    {
      title: "XiaomiROM",
      icon: "https://logo.clearbit.com/xiaomirom.com",
      desc: "小米手机ROM和刷机资源",
      link: "https://xiaomirom.com/",
    },
    {
      title: "Stack Overflow",
      icon: "https://logo.clearbit.com/stackoverflow.com",
      desc: "全球最大的程序员问答社区",
      link: "https://stackoverflow.com/",
    },
    {
      title: "VitePress | 快速上手中文教程",
      icon: "https://logo.clearbit.com/vitepress.yiov.top",
      desc: "VitePress静态站点生成器教程",
      link: "https://vitepress.yiov.top/",
    },
    {
      title: "Xhofe's Blog",
      icon: "https://logo.clearbit.com/blog.nn.ci",
      desc: "个人技术博客和开发分享",
      link: "https://blog.nn.ci/",
    },
    {
      title: "我不是咕咕鸽",
      icon: "https://logo.clearbit.com/blog.laoda.de",
      desc: "技术博客和开发经验分享",
      link: "https://blog.laoda.de/",
    },
    {
      title: "Yerongの小窝",
      icon: "https://logo.clearbit.com/www.coolhub.top",
      desc: "个人技术博客和开发分享",
      link: "https://www.coolhub.top/",
    },
    {
      title: "编程随想的博客",
      icon: "https://logo.clearbit.com/cirubla.github.io",
      desc: "编程技术博客和思考分享",
      link: "https://cirubla.github.io/",
    },
    ],
  },
  {
    title: '常用工具',
    items: [
      {
        icon: 'https://tinypng.com/images/apple-touch-icon.png',
        title: 'TinyPNG',
        desc: '在线图片压缩工具',
        link: 'https://tinypng.com',
      },
      {
        icon: 'https://carbon.now.sh/favicon.ico',
        title: 'Carbon',
        desc: '代码图片生成',
        link: 'https://carbon.now.sh',
      },
      {
        icon: 'https://npm.devtool.tech/logo.svg',
        title: 'npm 库在线执行',
        desc: '在浏览器控制台中在线运行调试与测试 npm 中的库',
        link: 'https://npm.devtool.tech',
      },
      {
        icon: 'https://devtool.tech/logo.svg',
        title: '开发者武器库',
        desc: '开发者武器库，做开发者最专业最好用的专业工具箱',
        link: 'https://devtool.tech',
      },
      {
        icon: 'https://carbon.now.sh/favicon.ico',
        title: 'tg问题',
        desc: '有Tg使用的问题或许可以在这里找到方案',
        link: 'https://tgnav.github.io/tgwiki',
      },
      {
        icon: 'https://squoosh.app/c/icon-demo-logo-326ed9b6.png',
        title: '在线png压缩',
        desc: 'png图片压缩',
        link: 'https://squoosh.app/',
      },
      {
        icon: 'https://uigradients.com/static/images/favicon-32x32.png',
        title: 'CSS渐变',
        desc: '在线查看渐变颜色,并生成css代码一键复制',
        link: 'https://uigradients.com/#Mini',
      },
      {
        icon: 'https://ezgif.com/favicon.ico',
        title: '在线转GIF',
        desc: '多种格式在线转换为GIF',
        link: 'https://ezgif.com/',
      },
    ],
  },
  // ---- 修正：为 “面试必备” 创建了一个新的分类对象 ----
  {
    title: '面试必备',
    items: [
      {
        title: '前端面试派',
        link: 'https://www.mianshipai.com/',
        desc: '双越老师联合精英博主 大厂面试流程开源免费',
        icon: 'https://www.mianshipai.com/favicon.ico',
      },
      {
        title: '代码随想录',
        link: 'https://programmercarl.com/',
        desc: '代码随想录，是一个关于算法与数据结构的博客',
        icon: 'https://code-thinking-1253855093.file.myqcloud.com/pics/20210614201246512.png',
      },
      {
        title: '小林 Coding',
        link: 'https://xiaolincoding.com/',
        desc: '图解计算机网络、操作系统、计算机组成、数据库等',
        icon: 'https://cdn.xiaolincoding.com/logo.png',
      },
      {
        title: '前端九部',
        link: 'https://www.yuque.com/fe9/basic',
        desc: '2019 年发展至今的前端知识体系',
        icon: 'https://mdn.alipayobjects.com/huamei_0prmtq/afts/img/A*vMxOQIh4KBMAAAAAAAAAAAAADvuFAQ/original',
      },
      {
        title: '面试官系列',
        link: 'https://vue3js.cn/interview/',
        desc: '一份通俗易懂的前端面试指南',
        icon: 'https://avatars.githubusercontent.com/u/9276376?v=4',
      },
    ],
  },
  {
    title: 'AI 时代',
    items: [
      {
        icon: 'https://cdn.oaistatic.com/assets/favicon-miwirzcw.ico',
        title: 'ChatGPT（最强）',
        link: 'https://chat.openai.com/chat',
        desc: 'OpenAI 推出的聊天机器人',
      },
      // ---- 修正：将 AI SDK 放入 AI 时代的 items 数组中 ----
      {
        title: 'AI SDK',
        link: 'https://sdk.vercel.ai/',
        desc: '面向前端开发人员的 AI 途径',
        icon: 'https://sdk.vercel.ai/favicon.ico',
      },
      {
        title: 'MagicEraser',
        icon: 'https://magiceraser.pro/favicon.ico',
        desc: 'AI 智能消除照片中多余物体路人工具，自动填充自然背景效果处理，保持画面整体美观且不留任何人工处理痕迹',
        link: 'https://magiceraser.pro/zh',
      },
    ],
  },
  // ---- 修正：为 “开源工具” 创建了一个新的分类对象 ----
  {
    title: '开源工具',
    items: [
      {
        title: 'Hello GitHub',
        icon: 'https://hellogithub.com/favicon.ico',
        desc: 'Find everything about GitHub',
        link: 'https://hellogithub.com',
      },
      {
        link: 'https://gitee.com/GrowingGit/GitHub-Chinese-Top-Charts#/GrowingGit/GitHub-Chinese-Top-Charts/blob/master/content/charts/overall/knowledge/JavaScript.md',
        title: 'GitHub中文排行榜',
        icon: 'https://gitee.com/assets/favicon.ico',
        desc: 'GitHub中文排行榜，帮助你发现高分优秀中文项目',
      },
      {
        title: 'FileCodeBox',
        link: 'https://fcb-docs.aiuo.net/',
        desc: '匿名口令分享文本，文件，像拿快递一样取文件',
        icon: 'https://fcb-docs.aiuo.net/logo_small.png',
      },
      {
        icon: 'https://tool.lu/favicon.ico',
        title: '在线工具',
        desc: '开发人员的工具箱',
        link: 'https://tool.lu',
      },
      {
        icon: 'https://telegra.ph/favicon.ico',
        title: 'Telegraph',
        desc: 'Telegram 的内容发布网站',
        link: 'https://telegra.ph',
      },
      {
        icon: 'https://transform.tools/static/favicon.png',
        title: 'transform',
        desc: '一个支持多语言的在线转换器',
        link: 'https://transform.tools/json-schema-to-typescript',
      },
    ],
  },
]
